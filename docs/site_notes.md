...
最後在 Docker 裡面跑`php artisan test --coverage` 之後，你就會看到你的`coverage-html`這個資料夾和`coverage-clover.xml`然後到`coverage-html`資料夾裡用簡單的靜態伺服器打開他即可，你可以使用`python3 -m http.server 8080 -d .`
