...
從一開始可以使用`php artisan make:auth` 做一個簡單的`auth`起手架以後，往後變成要下載package，到現在可以根據不同的情境使用不同的package。

真不知道該說這是變得麻煩還是越來越進步？(我私心覺得，是進步！)

1. Breeze

如果懷念以前的`make:auth`的時光可以使用這個package，附帶前端UI, 使用 Tailwind CSS 作為樣式配置

教學文件可以參考：[https://laravel.com/docs/9.x/starter-kits](https://laravel.com/docs/9.x/starter-kits)

2. Sanctum

我最近開了一個新的laravel專案，最近就有附帶這個package，他有點像是`firebase`那一種，總之如果你要簡單實作token based 的系統可以使用這個package，但他最大的缺點就是沒有支援`oauth`，如果要使用`oauth`請左轉去用`Passport`，否則他其實和`passport`很像的。
...
他比`Sanctum`早出生很多，或許在implement token based 以前專案都會使用這個套件，我自己目前還沒玩過(聽說未來我新公司會用，蠻期待的)。
...
這個套件有點神秘，他一個與其他auth相關套件最大的差別就是**他沒有前端實現**
