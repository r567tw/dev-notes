...
一種在應用層的比 HTTP 還輕量之通訊協議，常常被物聯網所使用。至於他的核心在於有一個名詞名為`Broker`，負責讓大家`subscribe`和`publish`。
...
其實網路上有很多教學再告訴你怎麼架設一個 MQTT Broker, 目前市面上分佈最廣的的是開源專案：`Mosquitto`, 不過我這裡為了方便示例與教學，就使用`Dockerfile`來快速啟動，而他也是`Mosquitto` 提供的 image。
...
之後再開一個新終端機或者另一個 tab 執行`publish.py`: `python publish.py`
